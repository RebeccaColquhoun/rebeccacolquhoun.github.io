---
layout: page
title: Teaching 
---

Teaching now forms the main component of my job as a Teaching Fellow in Geophysics and Quantitative Geoscience, with a particular interest in how we best teach maths and programming to students who may not have directly chosen to study them! I'm also interested in how we make education, including programming education, more accessible and inclusive for everyone.
## Imperial College London
### MAY 2025—PRESENT, Teaching Fellow in Geophysics and Quantitative Geoscience
*	2024—2025 Academic Year
    *	Conducted a systematic review of the geophysics curriculum.
    *	Courses taught:
  2nd year Field Geophysics (contributor, Summer 2025; 46 students)
*	Co-delivered London-based teaching including use of a range of geophysical analysis software and supported students with coursework. Marked coursework, notebooks and presentations.  
  2nd year introduction to Independent Geophysics Project (contributor, Summer 2025, 26 students)
    *	Other responsibilities:
  Worked with undergraduate admissions team on open days, taster days and outreach.
*	2025—2026 Academic Year
    *	Courses taught:
  Y3 60001b Independent Geophysics Project (coordinator, Autumn 2025; 28 students)
  Y2 50018 Numerical Methods (contributor (taught Linear Algebra), Spring 2026; 61 students)
  Y3/4 60041 Data Science and Machine Learning (contributor, Spring 2026; 57 students)
  Y3/4 70155 Planetary Physics (contributor (taught planetary magnetism), Spring 2026; 38 students)
  Y2 50006 Field Geophysics (contributor, Spring + Summer 2025; 62 students)
    *	Other responsibilities:
  Personal tutor (2025-2026: 15 students).
  EEDI committee member. 
  Athena SWAN SAT group member.
  Worked with undergraduate admissions team on interviews and open days.
  Assessor for 3 MSci projects. 
  City SPARC outreach.
### SEPTEMBER 2023—JUNE 2024 Teaching Fellow in Geophysics
*	Courses taught:
    *	1st year statistics
  Probability and probability distributions, statistical tests, correlation and linear regression.
  Redesigned course to be in Python. Wrote new problem sheets and worked answer keys.
    *	2nd year Applied Geophysics
  Edited and delivered course, designed and marked examination.
  Added a group presentation element to the course in response to student feedback. 
    *	2nd year Field Geophysics
  Taught on field trip with particular focus on the refraction seismology technique. 
  Co-delivered London-based teaching including use of a range of geophysical analysis software and supported students with coursework. Marked coursework, notebooks and presentations.  
  Organised speakers from the Centre for Academic English and Careers Service.
    *	3rd year Independent Geophysics Project
  Co-supervised projects. Marked and gave comprehensive feedback on each report. 
    *	3rd/4th year Planetary Physics
  Developed a new lecture on planetary magnetic fields. Contributed to writing and marking exam.
    *	Marking of 2 MSci projects
*	Other responsibilities:
    *	Personal tutor (11 students).
    *	Arranged academic tutorial program.
    *	EEDI committee member. Ran LGBTQIA+ History Month event.
    *	Worked with undergraduate admissions team on interviews and open days. Conducted data analysis to give new insight into applicant backgrounds and support widening participation efforts. Presented “Geophysics at Imperial” poster at Geophysics into Schools Meeting 2024.
## Graduate teaching and supervision, University of Oxford 
### JANUARY 2021—JUNE 2023, OCTOBER 2024—MAY 2025
*	The Oxford undergraduate course has a large component of small group teaching in tutorials. Giving tutorials involves setting questions and marking and giving feedback on work, including through writing termly reports to their main college tutors. I gave tutorials to a number of students (groups of 2--4) in: 
    *	1st year Mathematics (fundamentals of calculus, differential equations, linear algebra, statistics) 
    *	1st year Computing (MATLAB and Python3) 
    *	1st year Physics (mechanics, thermodynamics) 
    *	1st year Geophysics (gravity, plate tectonics, mantle dynamics) 
    *	2nd year Earthquakes and Faulting 
    *	3rd year Plate Tectonics 
*	Graduate teaching assistant for undergraduate courses
    *	1st Year Pembrokeshire Field Class
    *	1st Year Scientific Computing (Python3) for Earth Scientists 
    *	1st Year Statistics 
    *	2nd Year Geophysical Methods (gravity, seismic reflection and refraction) 
    *	2nd year Earthquakes and Faulting
*	 Master’s project supervision
    *	Co-supervised seismology master’s student with particular focus on helping student develop computational skills (Linux, Python3).
*	Demonstrating on graduate level courses
    *	Beginner Scientific Computing for Environmental Scientists (Python3; EPCC led, NERC funded)
    *	Intermediate Scientific Computing for Environmental Scientists (Python3; EPCC led, NERC funded)
## Qualifications
*  PGCert University Learning and Teaching (ongoing, Imperial College London)
*	University of Oxford Advancing Teaching and Learning (ATL) (2023). Led to AdvanceHE Associate Fellow status.
*	Mental Health First Aider
*	Fieldwork first aid
